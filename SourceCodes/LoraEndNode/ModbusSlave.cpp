...
uint16_t ModbusSlave::onSetHreg(TRegister* reg, uint16_t value) {
    ...
    // Handle MSG ID update
        ...
        uint16_t savedMsgID = instance->preferences.getUInt("msg_id", 0);  // Retrieve the saved MSG ID
        if (savedMsgID != value) {  // Only set the flag if the value is new
            Serial.printf("MSG ID updated from %d to %d. Triggering LoRa transmission...\n", savedMsgID, value);
            sendLoRaMessageFlag = true;  // Set the flag to trigger LoRa transmission
            instance->preferences.putUInt("msg_id", value);  // Save the new MSG ID
        } else {
            Serial.println("MSG ID unchanged. No action needed.");
        }
    }

    // Handle other registers
    else {
        uint16_t savedValue = instance->preferences.getUInt(String(address).c_str(), 0);
        if (savedValue != value) {
            Serial.printf("Register %d updated from %d to %d. Saving to NVS...\n", address, savedValue, value);
            instance->preferences.putUInt(String(address).c_str(), value);
        } else {
            Serial.printf("Register %d unchanged. Current value: %d\n", address, savedValue);
        }
    }
